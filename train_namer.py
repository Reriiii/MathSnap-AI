...
class Trainer:
    ...
    def train_epoch(self, loader, epoch: int, total_epochs: int,
        ...
        for step, batch in enumerate(pbar):
            ...
            vat_tgt, gt_coords = make_vat_targets(
            ...
            if use_gt_pgd:
                ...
                # Exact coordinates mapped tightly with GT sequences during Hungary Phase
                coords = gt_coords[:, :max_l].contiguous().float()
                
                # Special tokens mapping (SOS, EOS, PAD don't have physical coords)
                # Map them safely to left-center to avoid grid_sample artifacts
                sp_mask = (pgd_input == pad_idx) | (pgd_input == sos_idx) | (pgd_input == eos_idx)
                coords[sp_mask, 0] = map_h // 2
                coords[sp_mask, 1] = 0

                pos    = torch.arange(max_l, device=dev)
                # -1: convert COUNT → last valid INDEX (count=15 → index=14)
                ends   = ((pgd_input != pad_idx).sum(dim=1) - 1).clamp(0, max_l - 1)
                ...
                pos    = torch.arange(max_l, device=dev)
                # -1: convert COUNT → last valid INDEX
                ends   = ((pgd_input != pad_idx).sum(dim=1) - 1).clamp(0, max_l - 1)
                l_tgt2 = (pos - 1).clamp(min=0).unsqueeze(0).expand(B, -1).clone()
                r_tgt2 = (pos + 1).clamp(max=max_l - 1).unsqueeze(0).expand(B, -1).clone()
                for b in range(B):
