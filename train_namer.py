...
class Trainer:
    def __init__(self, model, optimizer, scheduler, loss_fn,
        ...
        # AMP — bf16 on RTX 5060 Ti (Blackwell) is ~2× faster than fp32
    ...
    def train_epoch(self, loader, epoch: int, total_epochs: int,
        ...
        for step, batch in enumerate(pbar):
            ...
            # If DWAP available → use teacher-forcing (1 forward pass, ~2.5× faster
            # than autoregressive decode). Paper Tab.4 shows position source barely
            # affects final ExpRate, so this trade-off is safe.
            T_pos = None
            ...
            # ── PGD forward (inside autocast for AMP) ────────────────────────
            with torch.amp.autocast(device_type=self.device.type, dtype=self.amp_dtype):
...
def run_training(cfg: Config):
    ...
    # Use cfg.num_workers for prefetching — keeps GPU fed between steps.
    # Windows users: set num_workers=0 if multiprocessing errors occur.
    dkw = dict(collate_fn=col, pin_memory=(device.type=='cuda'),
